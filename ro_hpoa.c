...
unsigned char *DAT_10022a68=(unsigned char*)0x80;
unsigned char *DAT_1000ff30=0;
unsigned char *DAT_10022c24=0;
...
// retrun value of flashput should be 0 and thus the return value of syste should be 0
  ...
  while( true ) {
    ...
    iVar9 = iVar9 + 1;
    if ((bVar1 || local_5c[0] != 0) || (((&read_buffer+1))[i] == '\0')) break;
    partition_nr = (unsigned int)(read_buffer+1)[i];
    partition_name = partition_selector(partition_nr);

    local_5c[0] = open_mtd_for_output_internal(fd,   partition_name,ret_offset);

    i = iVar18 * 0x15;
     
    if (local_5c[0] == 0) {
      //uVar3 = FUN_10002498((&local_247)[iVar5]);
      partition_nr = (unsigned int)(read_buffer+1)[i];
      //printf("partition_nr: %i\n",partition_nr);
      local_5c[0] = open_mtd_for_input_internal(partition_name,*(size_t *)(read_buffer+1 + i + 1),(void *)((int*)read_buffer +1 + i + 5));
...
void FUN_10006944(unsigned int *param_1,unsigned int *ABCD){
  ...
  DAT_10022a68=malloc(0x100*sizeof(char));
  memset(DAT_10022a68,(unsigned char)0x80,0x100);
  ...
  /* have to fix part of MD5 calc ... unsigned char* / unsigned int* thing */

  //printf("address of param_1: 0x%llX\n",(unsigned long long int*)param_1);
  //for(i=0;i<0x10;i++){
  //  printf("ABCD[%i]: 0x%X, ",i,*(ABCD+i));
  //  printf("param_1[%i]: 0x%llX\n",i,*((unsigned long long int*)(param_1+i)));
  //}
...
void MD5_encryption(unsigned int *variables,unsigned int *image_data,unsigned int len){
  ...
  // image data is processed ad int
  // should we devide len by 4?
  ...
  if (len < local_1c) {
    ...
    kind_of_memcpy((variables + local_18 + 18),(unsigned int*)image_data,local_1c);
    //printf("just after kind of memcpy\n");
    MD5_follow_precomputed_table((unsigned int*)variables,(unsigned int*)(variables + 6));
    //printf("just after MD5_follow...\n");
    for (; (local_1c + 0x3f) < len; local_1c = local_1c + 0x40) {
      //printf("len: 0x%X, local_1c: 0x%X\n",len,local_1c);
      MD5_follow_precomputed_table(variables,(unsigned int*)image_data + local_1c);
    }
    local_18 = 0;
  }
  //printf("at end local_1c (len?): %X\n", local_1c);
  //printf("at end local_18 (len?): %X\n", local_18);
  //printf("at end len(%i)-local_1c(%i): %i [decimal]\n",len, local_1c,len-local_1c);

  //(variable + 0x2E +18) = (variables+70), len =10
...
void old_FUN_CONCAT(unsigned int *p1,unsigned int *p2,uint len){
  unsigned char i=0;
  unsigned char j;
...
uint64_t FUN_1000ebfc(uint param_1,unsigned int  param_2,uint param_3,unsigned int param_4){
  ...
  FUN_1000e730(&local_28,auStack_88);
  FUN_1000e730(&local_20,auStack_68);
...
uint64_t FUN_1000f0f0(uint param_1,uint32_t param_2,uint param_3,uint32_t param_4){
  ...
  /* FUN_1000e730(uint param_1[2],uint param_2[6-8]) */
  FUN_1000e730(&local_38,local_78);
  FUN_1000e730(&local_30,local_58);
  puVar2 = local_78;
  uVar6 = local_78[3];
  uVar7 = local_78[4];
...
int open_mtd_for_input_internal(char *partition_name,int param_2,void *param_3){
  ...
  if (iVar2 == -1) {
    ...
    do {
      ...
      if (0 < sVar3) {
	MD5_encryption(auStack_88,data,sVar3);
        param_2 = param_2 - sVar3;
      }
    } while ((int)((-1 - sVar3) + (uint)(sVar3 == 0) & (-1 - param_2) + (uint)(param_2 == 0)) < 0);
...
LAB_10001f7c:
	  ...
	  /*problem with function below */
        ...
        if ((local_38[0] & 1) == 0) {
          //printf("\b\b\b\b%3d%%");
          printf("hier moet vaker over geitereerd worden check met 440... %3d%%\n");
          fflush(stdout);
        }
        fsync(__fd);
      } while (iVar7 < param_3);
...
int FUN_1000f7dc(unsigned int param_1,unsigned int param_2){
  ...
  FUN_1000e730(&local_18,local_38);
...
int get_em_type(void){
  ...
  if (DAT_000cbb58 == -1) {
    ...
    find_tag_in_file("gpio_states","OABOARDTYPE",&local_28,0x10,'=');
...
uint32_t FUN_10005de8(int param_1,unsigned char *param_2,unsigned char *param_3){
  ...
  unsigned char  local_57;
  unsigned char uStack_48;
  int local_47;
  ...
  if (((-1 < param_1) && (param_2 != (unsigned char *)0x0)) && (*param_2 - 1 < 2)) {
    ...
    if (uVar7 == 0) {
      if ((pbVar6 != (unsigned char *)0x0) && (*(int *)(pbVar6 + 8) != 0)) {
        sVar4 = FUN_10005d64(param_1,&uStack_58,0xe,&SStack_c8);
        if (sVar4 != 0) {
          return 0xffffffff;
        }
        uVar7 = 0;
        ...
        if (local_57 != 0) {
          do {
            iVar8 = iVar8 + 1;
            sVar4 = FUN_10005d64(param_1,&uStack_48,0x15,&SStack_c8);
            if (sVar4 != 0) {
              return 0xffffffff;
            }
            uVar7 = uVar7 + local_47;
          } while (iVar8 < (int)(uint)local_57);
        }
        uVar7 = FUN_10005cdc(param_1,uVar7,&SStack_c8);
        if (uVar7 != 0) {
          return 0xffffffff;
      ...
      sha256_final(param_3,&SStack_c8);
      uVar2 = 0;
    }
  }
  return uVar2;
