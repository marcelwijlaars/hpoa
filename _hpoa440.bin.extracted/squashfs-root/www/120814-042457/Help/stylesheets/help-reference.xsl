<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fn="http://www.w3.org/2005/02/xpath-functions">
  ...
  <xsl:param name="reference-id"><xsl:value-of select="0" /></xsl:param>
  ...
  <xsl:template match="category[@src]">
    <xsl:choose>
    <xsl:when test="@reference-id = $reference-id">
  ...
  <xsl:template match="topic[@reference-id]">
    <xsl:if test="@reference-id = $reference-id">	
		<div class="leafWrapper">
			<div class="leaf" id="{@id}">
				<a href="{@src}" target="iframeContent" class="treeSelectableLink">
					<img border="0" height="14" width="11" alt="" src="/120814-042457/Help/images/reference.gif" />
