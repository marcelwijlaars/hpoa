...
<xsl:stylesheet version="1.0"
	...
	<xsl:template name="groupSettings">

		<xsl:if test="count($enclosureList//enclosure[selected='true']) &gt; 1 and $action='add'">
          ...
          onclick="var elem=document.getElementById('overwriteExisting');elem.disabled=!this.checked;if(!this.checked)elem.checked=false;" />
			<label for="addToAllEnclosures">
        <xsl:value-of select="$stringsDoc//value[@key='addThisGroupToEnc']" /></label>
			<br />      
        <input style="margin: 10px 5px 0px 20px;" type="checkbox" id="overwriteExisting" class="stdCheckBox" disabled="disabled" />
          ...
          <xsl:value-of select="$stringsDoc//value[@key='overwriteSettingsExistGroups']" /></label>
      <br />
		...
		<table cellpadding="0" cellspacing="0" border="0" width="100%">
			<tr>
				<td>
          <xsl:value-of select="$stringsDoc//value[@key='groupInformation']" /><br /><span class="whiteSpacer">&#160;</span><br />
				</td>
				<xsl:if test="$username!='Administrator'">
					<td></td>
					<td>
            <xsl:value-of select="$stringsDoc//value[@key='groupPermissions']" /><xsl:if test="not($linkHasMixed = 'true')">: <em>
			...
			<tr>
				...
				<xsl:if test="$username!='Administrator'">
					...
					<td class="groupingBox" style="padding-left:10px;">
                ...
                <xsl:value-of select="$stringsDoc//value[@key='userSettingsFormPara1']" />
                <br />
                  ...
                  <xsl:value-of select="$stringsDoc//value[@key='oaBays']" /></label>
								...
								<xsl:element name="input">
									...
									<xsl:attribute name="onclick">
										checkboxToggle(this, 'devid', 'bay', '<xsl:value-of select="concat('svbSelectEnc', $encNum)" />');
									</xsl:attribute>
									<xsl:attribute name="type">checkbox</xsl:attribute>
                  ...
                  <xsl:value-of select="$stringsDoc//value[@key='allServerBays']" />
								...
								<xsl:element name="input">
									...
									<xsl:attribute name="onclick">
										checkboxToggle(this, 'devid', 'interconnect', '<xsl:value-of select="concat('swmSelectEnc', $encNum)" />');
									</xsl:attribute>
									<xsl:attribute name="type">checkbox</xsl:attribute>
									<xsl:attribute name="id">
