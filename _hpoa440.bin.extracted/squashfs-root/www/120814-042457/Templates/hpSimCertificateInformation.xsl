...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="*">
		...
		<xsl:value-of select="$stringsDoc//value[@key='hpSimCerts']" /><br />
		<span class="whiteSpacer">&#160;</span>
		<br />

		<xsl:choose>
			<xsl:when test="count($hpSimInfoDoc//hpoa:hpSimInfo/hpoa:certificates/hpoa:certificate) &gt; 0">

				<div class="errorDisplay" id="removeCertErrorDisplay"></div>

				<xsl:for-each select="$hpSimInfoDoc//hpoa:hpSimInfo/hpoa:certificates/hpoa:certificate">
								...
								<div>
									<div>
										<xsl:element name='button'>
											...
											<xsl:attribute name='onclick'>removeCertificate('<xsl:value-of select='hpoa:subjectCommonName'/>','<xsl:value-of select='hpoa:md5Fingerprint'/>');</xsl:attribute>
			...
			<xsl:otherwise>
				<div class="groupingBox">
					<xsl:value-of select="$stringsDoc//value[@key='noSimCertInstalled']" />
				</div>
			</xsl:otherwise>
		</xsl:choose>
