...
<xsl:stylesheet version="1.0"
	...
	<xsl:template name="bladeDevicePowerUsage">
				
		<table width="100%" class="captionTable" border="0" cellpadding="0" cellspacing="0">
			<tr>
				<td width="50%" align="left">Blade Device Power Usage</td>
						...
						<xsl:if test="hpoa:presence=$PRESENT">
							...
							<xsl:element name="tr">
								...
								<td width="20%">
									...
									<xsl:choose>
										<xsl:when test="hpoa:presence=$PRESENT">
											<xsl:element name="a">
												<xsl:attribute name="href">javascript:selectDevice(<xsl:value-of select="hpoa:bayNumber" />, BAY());</xsl:attribute>
								...
								<td width="20%" align="right">
									...
									<xsl:value-of select="$bladeThermalDoc//hpoa:thermalInfo[hpoa:bayNumber=$currentBayNumber]/hpoa:temperatureC"/>
								...
								<td width="20%" align="right" celltype="powerMeasurement">
									...
									<xsl:element name="div">
										...
										<xsl:attribute name="type">blade</xsl:attribute>
										<xsl:value-of select="hpoa:powerConsumed"/>
									</xsl:element>
								</td>
		...
		<table width="100%" border="0" class="dataTable" cellpadding="0" cellspacing="0">
			<tr class="summaryRow">
				<td>
					Total:&#160;<span id="bladePowerConsumed"></span>
				</td>
			</tr>
		</table>
