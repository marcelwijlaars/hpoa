...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="enclosureFrontView" match="*">
    ...
    <xsl:element name="table">
      ...
      <tr>
        <td colspan="2">
          <xsl:value-of select="$stringsDoc//value[@key='frontView']" /> - Top
        </td>
      </tr>
      ...
      <tr>
        ...
        <td rowspan="3">
          ...
          <table cellpadding="0" cellspacing="0" border="0">
            ...
            <tr>
              <!-- OA Tray -->
              <td title="Onboard Administrator Tray" class="deviceCellUnselectable" style="width:6px;height:224px;background-image:url('/120814-042457/images/oa_tray_c3000_t.gif');">
                <img src="/120814-042457/images/icon_transparent.gif" border="0" height="1" width="6" alt="" />
              </td>

              <td width="18" height="224">
									<table cellpadding="0" cellspacing="0" border="0">
										<!-- First OA table row -->
										<tr>
											<xsl:element name="td">
												...
												<xsl:attribute name="height">92</xsl:attribute>
												<xsl:attribute name="align">left</xsl:attribute>
												<xsl:attribute name="valign">top</xsl:attribute>
												<xsl:attribute name="style">line-height:92px;padding:0px;</xsl:attribute>
                        ...
                        <xsl:attribute name="bayNum">1</xsl:attribute>
														...
														<xsl:attribute name="background">
															<xsl:choose>
																<xsl:when test="$roleBay1 = $STANDBY">
																	<xsl:value-of select="$EM_PRESENT_IMAGE_STANDBY_C3000_T" />
																</xsl:when>
																<xsl:otherwise>
																	<xsl:value-of select="$EM_PRESENT_IMAGE_C3000_T" />
																</xsl:otherwise>
															</xsl:choose>
														</xsl:attribute>
												...
												<xsl:call-template name="emBay">
													...
													<xsl:with-param name="displayImage" select="$EM_PRESENT_IMAGE_C3000_T" />
												</xsl:call-template>
										...
										<tr>
											<xsl:element name="td">
												...
												<xsl:attribute name="style">background-image:url(<xsl:value-of select="$LCD_BLANK_IMAGE_C3000_T"/>);</xsl:attribute>
												...
												<img src="/120814-042457/images/icon_transparent.gif" border="0" height="38" width="16" alt="" />
                        ...
                        <xsl:attribute name="bayNum">2</xsl:attribute>
												...
												<xsl:choose>
													...
													<xsl:otherwise>
														<xsl:attribute name="class">deviceCellUnselectable</xsl:attribute>
														<xsl:attribute name="background">
															<xsl:value-of select="$EM_RESERVED_IMAGE_C3000_T" />
														</xsl:attribute>
														<img src="/120814-042457/images/icon_transparent.gif" border="0" height="1" width="16" alt="" />
													</xsl:otherwise>
												</xsl:choose>
											</xsl:element>
										</tr>
        ...
        <td width="110" height="18" class="deviceCellUnselectable" background="/120814-042457/images/leftside_c3000_t.gif">
          <img src="/120814-042457/images/icon_transparent.gif" border="0" height="18" width="1" alt="" />
        </td>
							...
							<xsl:for-each select="$domainInfoDoc//hpoa:domainInfo/hpoa:domains/hpoa:domain[hpoa:size=1 or hpoa:size=2 or hpoa:size=4]">
								...
								<xsl:element name="div">
									...
									<xsl:variable name="x_pos">
										<xsl:choose>
											...
											<xsl:when test="$firstBay=2">
												<xsl:value-of select="(28*2)"/>
											</xsl:when>
											<xsl:when test="$firstBay=3">
												<xsl:value-of select="(28*2)"/>
											</xsl:when>
											<xsl:when test="$firstBay=4">
												<xsl:value-of select="(28*3)"/>
											</xsl:when>
										</xsl:choose>
									</xsl:variable>
									...
									<xsl:choose>
										<xsl:when test="hpoa:size=4">
											<xsl:attribute name="style">
												z-index:100; position:absolute; left:<xsl:value-of select="$x_pos" />px; top:49px; width:<xsl:value-of select="$width" />px; height72:px; background:url('/120814-042457/images/domain_bar_4_90_t.gif');
											</xsl:attribute>
										</xsl:when>
										<xsl:when test="hpoa:size=2">
											<xsl:attribute name="style">
												z-index:100; position:absolute; left:<xsl:value-of select="$x_pos" />px; top:49px; width:<xsl:value-of select="$width" />px; height72:px; background:url('/120814-042457/images/domain_bar_2_90_t.gif');
											</xsl:attribute>
										</xsl:when>
										<xsl:when test="hpoa:size=1">
											<xsl:attribute name="style">
												z-index:100; position:absolute; left:<xsl:value-of select="$x_pos" />px; top:49px; width:<xsl:value-of select="$width" />px; height72:px; background:url('/120814-042457/images/domain_bar_1_90_t.gif');
											</xsl:attribute>
										</xsl:when>
									</xsl:choose>
									...
									<table cellspacing="0" cellpadding="0" border="0">
										<tr>
											<td id="{concat('enc', $encNum, 'monarch', hpoa:monarchBlade, 'Select')}" devNum="{hpoa:monarchBlade}" style="width:{$width}px;height:72px;" encNum="{$encNum}" devType="monarch" class="deviceCell">
      ...
      <tr>
          ...
          <xsl:element name="td">
			  ...
			  <xsl:variable name="dvdImage">
				  <xsl:choose>
					  <xsl:when test="count($oaMediaDeviceList//hpoa:oaMediaDevice[hpoa:deviceType=1 and hpoa:devicePresence='PRESENT']) &gt; 0">
						  <xsl:value-of select="$MEDIA_DVD_PRESENT_c3000_T"/>
					  </xsl:when>
					  <xsl:otherwise>
						  <xsl:value-of select="$MEDIA_DVD_NOT_PRESENT_c3000_T"/>
					  </xsl:otherwise>
				  </xsl:choose>
			  </xsl:variable>
            ...
            <xsl:element name="div">
              <xsl:attribute name="style">position:relative;border:0px;width:104px;height:18px;</xsl:attribute>
  ...
  -->
    ...
    <xsl:variable name="healthStatus">
      <xsl:choose>
		...
		<xsl:variable name="left">
			...
			<xsl:choose>
				<xsl:when test="number($physicalBayNumber) &lt;=$BLADES_PER_ROW">
					<xsl:choose>
						<!-- Special case for Double Dense B side -->
						<xsl:when test="number(hpoa:extraData[@hpoa:name='NormalizedSideNumber'])=2">
							<xsl:value-of select="((number($physicalBayNumber)-1)*28)+$DD_BAY_WIDTH+1"/>
						</xsl:when>
						<xsl:otherwise>
							<!-- Special case for double-wide blades -->
							<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:choose>
						<!-- Special case for Double Dense B side -->
						<xsl:when test="number(hpoa:extraData[@hpoa:name='NormalizedSideNumber'])=2">
							<xsl:value-of select="((number($physicalBayNumber)-5)*28)+$DD_BAY_WIDTH+1"/>
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="((number($physicalBayNumber)-5)*28)+1"/>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:otherwise>
			</xsl:choose>
		...
		<xsl:variable name="top">
			<xsl:choose>
				<xsl:when test="$physicalBayNumber &gt; $BLADES_PER_ROW">93</xsl:when>
		...
		<xsl:variable name="position">
			<xsl:choose>
				<xsl:when test="$physicalBayNumber &lt;= $BLADES_PER_ROW">TOP</xsl:when>
				...
				<xsl:when test="$height=1 or $height=0">
						...
						<xsl:when test="hpoa:deviceId='82'">
							<xsl:choose>
								<xsl:when test="hpoa:productId='8213'">
									<xsl:value-of select="$BLADE_IO_AMC_IMAGE_90_T"/>
								</xsl:when>
								<xsl:when test="hpoa:productId='8211' and not(contains(hpoa:name, 'PCI'))">
									<xsl:value-of select="$BLADE_IO_XW_GRAPHICS_90_T"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="$BLADE_STORAGE_IO_EXPANSION_IMAGE_90_T" />
								</xsl:otherwise>
							</xsl:choose>
						</xsl:when>
                  ...
                  <xsl:choose>
                    <xsl:when test="contains($bladeName, 'D2220sb') or contains($bladeName, 'G2')">
                      <xsl:value-of select='"/120814-042457/images/d2200sbG2_90_t.gif"'/>
                    </xsl:when>
                    <xsl:otherwise>
                      <xsl:value-of select='"/120814-042457/images/d2200sb_90_t.gif"'/>
                    </xsl:otherwise>
                  </xsl:choose>
								</xsl:when>
								<xsl:when test="hpoa:productId='8210'">
									<xsl:value-of select="$BLADE_STORAGE_TAPE_IMAGE_90_T" />
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="$BLADE_STORAGE_IMAGE_90_T" />
								</xsl:otherwise>
							</xsl:choose>
						</xsl:when>
						<xsl:when test="$bladeType='BLADE_TYPE_SERVER'">
							<xsl:choose>
								<xsl:when test="number($bayNumber)&gt;16 and number($bayNumber)&lt;=32">
									<xsl:value-of select="$BLADE_HALF_IMAGE_2X220_SIDE_A_90_T" />
								</xsl:when>
								<xsl:when test="number($bayNumber)&gt;32 and number($bayNumber)&lt;=48">
									<xsl:value-of select="$BLADE_HALF_IMAGE_2X220_SIDE_B_90_T" />
								</xsl:when>
                <xsl:when test="contains($bladeName, 'Gen9')">
                  <xsl:choose>
                    <xsl:when test="contains($bladeName, 'BL460c') or contains($bladeName, 'WS460c')">
                      <xsl:value-of select="$BLADE_IMAGE_BL460C_G9_90_T"/>
                    </xsl:when>
                    <xsl:otherwise>
                      <xsl:value-of select="$BLADE_UNKNOWN_IMAGE_90_T" />
                    </xsl:otherwise>
                  </xsl:choose>                  
                </xsl:when>
                <xsl:when test="contains($bladeName, 'Gen8') or contains($bladeName, 'StoreEasy 38') or contains($bladeName, 'FlexServer')">
                  <xsl:choose>
                    <xsl:when test="contains($bladeName, 'BL420c')">
                      <xsl:value-of select="$BLADE_IMAGE_BL420C_G8_90_T"/>
                    </xsl:when>
                    <xsl:when test="contains($bladeName, 'BL460c') or contains($bladeName, 'WS460c') or contains($bladeName, 'StoreEasy 38') or contains($bladeName, 'FlexServer B390')">
                      <xsl:value-of select="$BLADE_IMAGE_BL460C_G8_90_T"/>
                    </xsl:when>
                    <xsl:when test="contains($bladeName, 'BL465c')">
                      <xsl:value-of select="$BLADE_IMAGE_BL465C_G8_90_T"/>
                    </xsl:when>
                    <xsl:otherwise>
                      <xsl:value-of select="$BLADE_UNKNOWN_IMAGE_90_T" />
                    </xsl:otherwise>
								...
								<xsl:when test="contains($bladeName, 'BL260') or contains($bladeName, 'BL280')">
									<xsl:value-of select="$BLADE_IMAGE_BL260C_90_T"/>
								</xsl:when>
								<xsl:when test="contains($bladeName, 'BL495') or contains($bladeName, 'BL490')">
									<xsl:value-of select="$BLADE_IMAGE_BL495C_90_T"/>
								</xsl:when>
								<xsl:when test="contains($bladeName, 'BL460') and contains($bladeName, 'G6')">
									<xsl:value-of select="$BLADE_IMAGE_BL460CG6_90_T"/>
								</xsl:when>
								<xsl:when test="contains($bladeName, 'BL465') and contains($bladeName, 'G7')">
									<xsl:value-of select="$BLADE_IMAGE_BL465CG7_90_T"/>
								</xsl:when>
								<xsl:when test="$height=1">
									<xsl:value-of select="$BLADE_HALF_IMAGE_90_T" />
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="$BLADE_UNKNOWN_IMAGE_90_T" />
								</xsl:otherwise>
						...
						<xsl:when test="hpoa:deviceId='72' or hpoa:productId='4609'">
							<xsl:choose>
								<xsl:when test="contains($bladeName, '870')">
									<xsl:value-of select="$BLADE_FULL_IMAGE_870_90_T" />
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="$BLADE_FULL_IMAGE_IA64_90_T" />
								</xsl:otherwise>
							</xsl:choose>
						</xsl:when>
						<xsl:when test="hpoa:deviceId='51' and hpoa:productId='33282'">
							<xsl:value-of select='"/120814-042457/images/enclosure_blade_BL860c_g7_90_t.gif"' />
						</xsl:when>
            <xsl:when test="contains($bladeName, 'Gen9')">
              <xsl:choose>
                <xsl:when test="contains($bladeName, 'BL660c')">
                  <xsl:value-of select="$BLADE_IMAGE_BL660C_G9_90_T"/>
                </xsl:when>
              </xsl:choose>                  
            </xsl:when>
            <xsl:when test="contains($bladeName, 'Gen8') or contains($bladeName, 'FlexServer')">
              <xsl:choose>
                <xsl:when test="contains($bladeName, 'BL685c')">
                  <xsl:value-of select="$BLADE_IMAGE_BL685C_G8_90_T"/>
                </xsl:when>
                <xsl:when test="contains($bladeName, 'BL660c') or contains($bladeName, 'FlexServer B590')">
                  <xsl:value-of select="$BLADE_IMAGE_BL660C_G8_90_T"/>
                </xsl:when>
              </xsl:choose>                  
            </xsl:when>
						<xsl:when test="contains($bladeName, '480')">
							<xsl:value-of select="$BLADE_FULL_IMAGE_I_90_T" />
						</xsl:when>
						<xsl:when test="contains($bladeName, '680') and not(contains($bladeName, 'G7'))">
							<xsl:value-of select="$BLADE_FULL_IMAGE_I4_90_T" />
						</xsl:when>
						<xsl:when test="contains($bladeName, '680') and contains($bladeName, 'G7')">
							<xsl:value-of select='"/120814-042457/images/enclosure_blade_680c_g7_4_90_t.gif"'/>
						</xsl:when>
						<xsl:when test="contains($bladeName, '685') or contains($bladeName, '620')">
							<xsl:choose>
								<xsl:when test="contains($bladeName, 'G6') or contains($bladeName, 'G7')">
									<xsl:value-of select="$BLADE_FULL_IMAGE_A_G6_90_T" />
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="$BLADE_FULL_IMAGE_A_90_T" />
								</xsl:otherwise>
							</xsl:choose>
						</xsl:when>
						<xsl:otherwise>
	...
	<xsl:template name="enclosureBackground">
		...
		<table style="background-color:#333; width:112px; height:184px;" cellpadding="0" cellspacing="0" border="0">
			<tr>
				<!-- bays 1,2,3,4 -->
				<td class="deviceCellUnselectable" style="width:26px;height:90px; vertical-align:top;">
      ...
      <xsl:when test="($bladeInfoDoc//hpoa:bladeInfo[hpoa:bayNumber=$bayNumber+16]/hpoa:height!=0 or $bladeInfoDoc//hpoa:bladeInfo[hpoa:bayNumber=$bayNumber+32]/hpoa:height!=0)">
        <xsl:choose>
          <xsl:when test="($currentUserInfo//hpoa:bladeBays/hpoa:blade[hpoa:bayNumber=$bayNumber]/hpoa:access='false' and $currentUserInfo//hpoa:bladeBays/hpoa:blade[hpoa:bayNumber=$bayNumber+16]/hpoa:access='false' and $currentUserInfo//hpoa:bladeBays/hpoa:blade[hpoa:bayNumber=$bayNumber+32]/hpoa:access='false')">
            <img src="/120814-042457/images/icon_lock_16.gif" style="margin-top: {$vOffset}; margin-left: 7px;" />
          </xsl:when>
          ...
          <xsl:when test="$currentUserInfo//hpoa:bladeBays/hpoa:blade[hpoa:bayNumber=$bayNumber+16]/hpoa:access='true' and $currentUserInfo//hpoa:bladeBays/hpoa:blade[hpoa:bayNumber=$bayNumber+32]/hpoa:access='false'">
            <img src="/120814-042457/images/icon_lock_16.gif" style="margin-top: {$vOffset}; margin-left: 13px;" />
          </xsl:when>
          <xsl:when test="$currentUserInfo//hpoa:bladeBays/hpoa:blade[hpoa:bayNumber=$bayNumber+16]/hpoa:access='false' and $currentUserInfo//hpoa:bladeBays/hpoa:blade[hpoa:bayNumber=$bayNumber+32]/hpoa:access='true'">
            <img src="/120814-042457/images/icon_lock_16.gif" style="margin-top: {$vOffset}; margin-left: 2px;" />
          </xsl:when>
