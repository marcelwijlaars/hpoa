...
<xsl:stylesheet version="1.0"
	...
	<xsl:template name="certificateUpload" match="*">
		...
		<em>
		<xsl:value-of select="$stringsDoc//value[@key='x509CertUploadDesc']" />
		</em><br />
		...
		<div align="right">
			<div class='buttonSet' style="margin-bottom:0px;">
				<div class='bWrapperUp'>
					<div>
						<div>
							<button type='button' class='hpButton' onclick='uploadCertificate();'>
							<xsl:value-of select="$stringsDoc//value[@key='upload']" /></button>
						</div>
					</div>
				</div>
			</div>
		...
		<xsl:value-of select="$stringsDoc//value[@key='downloadFromURL']" />&#160;
		<em>
		<xsl:value-of select="$stringsDoc//value[@key='downloadFromURLDesc']" />
		</em><br />
		<span class="whiteSpacer">&#160;</span><br />
		...
		<em>
                <xsl:value-of select="$stringsDoc//value[@key='downloadFromURLNotice']" />
                </em><br />
