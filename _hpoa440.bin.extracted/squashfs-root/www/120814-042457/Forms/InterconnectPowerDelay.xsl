...
<xsl:stylesheet version="1.0"
	...
	<xsl:param name="interconnectStatusDoc" />
	...
	<xsl:template match="*">
		...
		<table align="center" border="0" cellpadding="0" cellspacing="0" class="dataTable" id="powerDelayContainer">
			...
			<TBODY>
				...
				<xsl:choose>
					<xsl:when test="count($powerSequenceDoc//hpoa:powerdelaySettings/hpoa:interconnects/hpoa:bays) &gt; 0">
						<xsl:for-each select="$powerSequenceDoc//hpoa:powerdelaySettings/hpoa:interconnects/hpoa:bays">
							...
							<xsl:element name="tr">
								...
								<td id="{concat('bay', $bayNumber, 'Label')}">
									...
									<xsl:variable name="presence" select="$interconnectStatusDoc//hpoa:interconnectTrayStatus[hpoa:bayNumber=$bayNumber]/hpoa:presence" />
									...
									<xsl:choose>
										...
										<xsl:when test="$presence=$SUBSUMED">
										...
										<xsl:when test="$serviceUserAcl = $ADMINISTRATOR">
											<xsl:element name="input">
												...
												<xsl:attribute name="id">
													<xsl:value-of select="concat('delayInput', $bayNumber)"/>
												</xsl:attribute>
			...
			<div align="right">
				<div class='buttonSet'>
					<div class='bWrapperUp'>
						<div>
							<div>
								<button type='button' onclick="setPowerDelaySettings('interconnect');" class='hpButton' id="btnApply">
